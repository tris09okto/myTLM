...
export async function POST(request: Request) {
  try {
    ...
    const [leaders] = await pool.query(
      `SELECT * FROM data_non_fronting WHERE username = ?`,
      [username]
    ) as [any[], any];

    if (leaders.length === 0) {
      return NextResponse.json({ success: false, message: 'Akun supervisor tidak ditemukan.' }, { status: 404 });
    }
    ...
    await pool.query(
      `UPDATE data_non_fronting SET password = ? WHERE username = ?`,
    ...
    console.error('Database Error pada /api/spv/change-password:', error);
